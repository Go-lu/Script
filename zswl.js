...
async function readNews() {
    ...
    try {
        let url = {
            url: `${hostname}/api/article/detail?from_channel=5bd2bef6b1985042e7691870&id=${id}`,
            headers: {
                'X-SESSION-ID': ck[0],
                'X-REQUEST-ID': ck[1],
                'X-TIMESTAMP': ts,
...
async function comments() {
    ...
    try {
        let url = {
            ...
            form: {
                "channel_article_id": id, //此处需要修改
                "content": "%E5%AD%A6%E4%B9%A0%E5%AD%A6%E4%B9%A0%EF%BC%81"
            }
        };
        let result = await httpPost(url, name);
...
async function channel_id() {
    ...
    try {
        let url = {
            url: `${hostname}/api/article/channel_list?channel_id=5bd2bef6b1985042e7691870&isDiFangHao=false&is_new=true&list_count=0&size=10`,
            headers: {
                'X-SESSION-ID': ck[0],
                'X-REQUEST-ID': ck[1],
                'X-TIMESTAMP': ts,
        ...
        if (result.code === 0) {
            let article_list = result["data"]["article_list"];
            DoubleLog(`${name}: 成功`);
            for (let i = 0; i < article_list.length; i++) {
                if (article_list[i]["mlf_id"] !== 0 && article_list[i]["list_title"] !== "") {
                    // 返回文章id
                    return article_list[i]["id"];
                }
            }
            return 0;
        } else {
